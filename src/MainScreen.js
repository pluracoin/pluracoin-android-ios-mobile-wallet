...
export function sendNotification(transaction) {
    ...
    PushNotification.localNotification({
        title: `Received ${prettyPrintAmount(transaction.totalAmount(), Config)}`,
...
export class MainScreen extends React.Component {
    ...
    async updateBalance() {
        ...
        let tmpPrice;
        if(Globals.coinPriceLastUpdate+60<now) {
            tmpPrice = await getCoinPriceFromAPI();
        }
        Globals.coinPriceLastUpdate = now;
        ...
        return(
            <View style={{ alignItems: 'center' }}>
                <Text style={[Styles.centeredText, {
                    color: this.props.screenProps.theme.whiteColour,
                    fontSize: 20,
                    marginBottom: 15,
                }]}>
                    Your Wallet Address:
...
class BalanceComponent extends React.Component {
    ...
    render() {
        const compactBalance = <OneLineText
                                     style={{ color: this.props.lockedBalance === 0 ? this.props.screenProps.theme.primaryColour : 'orange', fontSize: 30}}
        ...
        return(
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', bottom: 20, position: 'absolute', left: 0, right: 0 }}>
                <Animatable.Text ref={this.syncRef} style={{
                   ...
                   Sync {this.state.percent}% ({this.state.walletHeight} / {this.state.networkHeight})
                </Animatable.Text>
            </View>
        );
    }
